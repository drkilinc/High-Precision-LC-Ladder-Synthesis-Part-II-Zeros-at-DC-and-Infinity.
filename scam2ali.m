function [pay,payda]=scam2ali(cirtext)
% This program has designet to calculate the input impedance of any given 
% circuit composed from R, L, C components.
% Ali KILINC, 15-08-2007
% 
%  This program derived from SCAM available at
...
fid=fopen('cir.txt','w');
for i=1:length(cirtext)
    c1=char(cirtext(i));
    fprintf(fid,'%s\r\n',c1);
end
fclose(fid);

[Name N1 N2 arg3]=textread('cir.txt','%s %s %s %s ');   
...
% add an independent voltege source to input
numV=numV+1;
Vsource(numV).Name='I1';
Vsource(numV).Node1=1;%str2num(N1{i});
Vsource(numV).Node2=0;%str2num(N2{i});
Vsource(numV).Value=1;%str2num(arg3{i});
...
for i=1:numElem,
    ...
    switch(Element(i).Name(1)),
        case 'R',
%             g = ['1/' Element(i).Name];
            g = ['1/' num2str(Element(i).Value)];
        case 'L',
%             g = ['1/s/' Element(i).Name];
            g = ['1/s/' num2str(Element(i).Value)];
        case 'C',
%             g = ['s*' Element(i).Name];
            g = ['s*' num2str(Element(i).Value)];
...
%[zns1,zds1]=numden(simplify(-v_1/I_V1));
[zns1,zds1]=numden(simplify(-v_1/I_I1));
 
pay=sym2poly(zns1);
payda=sym2poly(zds1);

if pay(end) > payda(end)
    k=pay(end);
else
    k=payda(end);
end
